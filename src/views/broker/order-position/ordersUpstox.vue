<template>
  <Modal1 size="5xl" :show="showUpstoxModal" @close ="closeModel" :showHeader="false">


    <template #body>
      <div class="flex px-3 py-2 text-lg font-medium justify-between ">
        <div>Upstox ({{ typeofdata }})</div>
        <div class="flex">
            <div
        ...
        <table class="w-full table table-report -mt-2 dark:text-white" v-if="showPositionsbol">
          <thead>
            <tr class="text-start">
              ...
              <th class="text-center whitespace-nowrap">UNREALISED</th>
          ...
          <tbody>
            ...
            <td>{{ order.pnl }}</td>
            <td class="text-center">{{ order.realised }}</td>
            <td class="text-center">{{ order.unrealised }}</td>
...
const showUpstoxModal = ref<boolean>(false)
watchEffect(() => {
  showUpstoxModal.value = showOrderPositionModal.value && showBrokerName.value === "upstox"
})
