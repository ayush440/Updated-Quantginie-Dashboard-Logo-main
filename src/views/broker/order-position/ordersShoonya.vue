<template>
  <Modal1 size="5xl" :show="showShoonyaModal" @close ="closeModel" :showHeader="false">


    <template #body>
      <div class="flex px-3 py-2 text-lg font-medium justify-between ">
        <div>Shoonya ({{ typeofdata }})</div>
        <div class="flex">
            <div
...
const orders = computed(() => {
    let order
    if (brokerOrders.value && brokerOrders.value){
...
const positions = computed(() => {
  let pos
  if (brokerPositions.value && brokerPositions.value){
...
const showShoonyaModal = ref<boolean>(false)
watchEffect(() => {
  showShoonyaModal.value = showOrderPositionModal.value && showBrokerName.value === "shoonya"
})
// const showShoonyaModal = computed(() => {
