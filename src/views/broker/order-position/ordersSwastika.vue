<template>
  <Modal1 size="5xl" :show="showSwastikaModal" @close ="closeModel" :showHeader="false">


    <template #body>
      <div class="flex px-3 py-2 text-lg font-medium justify-between ">
        <div>Swastika ({{ typeofdata }})</div>
        <div class="flex">
            <div
      ...
      <div class="table-container-modal text-nowrap intro-y">
        <table class="-mt-2 dark:text-white" v-if="showOrdersbol">
          ...
          <tbody>
              ...
              >
                <td>{{ order.Norentm }}</td>
                <td>{{ order.Tsym }} ({{ order.Exch }})</td>
                <td>
                  <span v-if="order.Trantype == 'B'">Buy</span>
                  <span v-else-if="order.Trantype == 'S'">Sell</span>
                  <span v-else>{{ order.Trantype }}</span>
                </td>
                <td>{{ order.Prctyp }}</td>
                <td>{{ order.Prc }}</td>
        ...
        <table class="w-full table table-report -mt-2 dark:text-white" v-if="showPositionsbol">
          <thead>
            <tr class="text-start">
              ...
              <th class="whitespace-nowrap">LTP</th>
              <th class="whitespace-nowrap">PNL</th>
          ...
          <tbody>
            ...
            <td>{{ order.Tsym }}</td>
            <td>
              <span v-if="order.Prd=='I'">Intraday</span>
              <span v-else-if="order.Prd=='D'">Delivery </span>
              <span v-else-if="order.Prd=='M'">Margin </span>
              <span v-else>{{ order.Prd }}</span>
            </td>
            <td>{{ order.Netqty }}</td>
            <td>{{ order.Lp }}</td>
            <td>{{ order.Rpnl }}</td>
...
const positions = computed(() => {
  ...
  if (brokerPositions.value && brokerPositions.value.Data){
    pos = brokerPositions.value.Data;
  }else{
    pos = []
  } 
...
const showSwastikaModal = ref<boolean>(false)
watchEffect(() => {
  showSwastikaModal.value = showOrderPositionModal.value && showBrokerName.value === "swastika"
})
// const showSwastikaModal = computed(() => {
...
function openSqOffModal(posData: any) {
  ...
  if(posData.Netqty > 0){
    ...
    brokerPosSqOffData.value.exchange = posData.Exch
    brokerPosSqOffData.value.tradingsymbol = posData.Tsym
    brokerPosSqOffData.value.product = posData.Prd
