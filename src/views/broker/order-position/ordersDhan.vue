<template>
  <Modal1 size="5xl" :show="showDhanModal" @close ="closeModel" :showHeader="false">


    <template #body>
      <div class="flex px-3 py-2 text-lg font-medium justify-between ">
        <div>Dhan ({{ typeofdata }})</div>
        <div class="flex">
            <div
      ...
      <div class="table-container-modal text-nowrap intro-y">
        <table class="-mt-2 dark:text-white" v-if="showOrdersbol">
          <thead>
            <tr class="text-start">
              ...
              <th class="whitespace-nowrap">TICKER</th>
          ...
          <tbody>
              ...
              >
                ...
                <td>{{ order.tradingSymbol }} ({{ order.exchangeSegment }})</td>
                <td>{{ order.transactionType }}</td>
                <td>{{ order.orderType }}/{{ order.productType }}</td>
                <td>{{ order.price }}</td>
                <td>{{ order.filled_qty }}/{{ order.quantity }}</td>
        ...
        <table class="w-full table table-report -mt-2 dark:text-white" v-if="showPositionsbol">
          <thead>
            <tr class="text-start">
              <th class="whitespace-nowrap">TRADING SYMBOL	</th>
              <th class="whitespace-nowrap">PRODUCT	</th>
              <th class="whitespace-nowrap">BUY QTY</th>
              <th class="whitespace-nowrap">SELL QTY</th>
              <th class="whitespace-nowrap">NET QTY</th>
              <th class="whitespace-nowrap">REALISED</th>
              <th class="whitespace-nowrap">UN REALISED</th>
          ...
          <tbody>
            ...
            <td>{{ order.dayBuyQty }}/{{ order.buyAvg }}</td>
            <td>{{ order.daySellQty }}/{{ order.sellAvg }}</td>
            <td>{{ order.netQty }}</td>
            <td>{{ order.realizedProfit }}</td>
...
const showDhanModal = ref<boolean>(false)
watchEffect(() => {
  showDhanModal.value = showOrderPositionModal.value && showBrokerName.value === "dhan"
})
// const showDhanModal = computed(() => {
...
function openSqOffModal(posData: any) {
  ...
  if(posData.netQty > 0){
    ...
    brokerPosSqOffData.value.instrument_token = typeof posData.securityId === "string" ? posData.securityId : String(posData.securityId)
