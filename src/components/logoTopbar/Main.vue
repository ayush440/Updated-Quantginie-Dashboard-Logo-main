<template>
  <!-- logo-topbar -->
  <div
    class="logo-topbar flex justify-between items-center space-x-4 relative py-4 px-6 bg-white dark:bg-[#1F242A]"
  >
    <RouterLink
      to="/"
      class="top-logo flex items-center justify-center xl:justify-start rounded-lg overflow-hidden"
    >
      <span
        class="unselectable capitalize font-bold text-2xl text-[#30437C] dark:text-white text-[30px]"
        >QuantGinie</span
      >
    ...
    <div class="flex items-center">
      <DarkModeSwicher />
      <div
        class="cursor-pointer ml-2 hover:bg-third-bold rounded-lg transition duration-200 dark:text-tableText"
        :class="{ 'rotate-0': !toggle, 'rotate-[180deg]': toggle }"
      >
        <MenuIcon
          v-if="!toggle"
          class="m-2 w-7 h-7 transition transition-delay-300 hover:scale-110"
  ...
  <teleport to="body">
    <div class="visible md:hidden h-full text-black dark:text-tableText bg-secondary">
      <transition name="slide-fade">
        <div
          v-if="toggle"
          @click.self="closeLogoTopbar"
          class="fixed h-full w-full menuBar overflow-y-scroll top-0 left-0"
          :style="{ zIndex: zIndexValue + 2 }"
        >
          <div
            ref="logoTopbar"
            class="bg-secondary overflow-y-auto overflow-x-hidden h-full menuBar-shadow w-[250px] transform"
          ...
          >
            ...
            <div class="pb-4 ml-1 flex flex-col shadow">
              <PopupDropdown
                ...
                dropdownClass="dropdown-1 bg-secondary top-[100%] left-[8px] w-[220px] cursor-pointer shadow-lg border border-secondary m-1"
              >
                    ...
                    <div class="flex items-center cursor-pointer" @click="toggleProfileModal">
                      ...
                      <div class="ml-2">
                        ...
                        <div class="text-gray-500 dark:text-tableText">
                          {{ profile?.user_role }}
                          <RightArrowIcon
                            class="w-[14px] h-[14px] inline ml-1 transform rotate-90 transition transition-delay-300"
                            :class="{
                              'rotate-[-90deg]': showProfileModal,
                              'rotate-90': !showProfileModal
                            }"
                          />
                        </div>
                      </div>
              ...
              <PopupDropdown
                ...
                dropdownClass="dropdown-1 bg-secondary top-[100%] left-[5%] w-[230px] shadow-lg border border-secondary"
              >
                <template #dropdown-toggle>
                  <div
                    @click="toggleNotificationModal"
                    class="unselectable flex flex-col cursor-pointer p-2 pt-4 pl-2"
                  >
                    <div class="flex items-center cursor-pointer font-semibold text-lg">
                      ...
                      <RightArrowIcon
                        class="w-[14px] h-[14px] inline ml-1 transform rotate-90 transition transition-delay-300"
                        :class="{
                          'rotate-[-90deg]': showNotificationModal,
                          'rotate-90': !showNotificationModal
                        }"
                      />
                    </div>
                  </div>
                ...
                <template #dropdown-body>
                  <div
                    class="max-h-[320px] text-sm overflow-y-scroll text-gray-600 dark:text-white text-center p-2"
                  >
                    <div
                    ...
                    >
                      <div class="ml-2 overflow-hidden">
                        ...
                        <div
                          class="text-left text-wrap text-xs sm:text-sm truncate text-slate-400 mt-0.5"
                        >
            ...
            <ul
              class="flex flex-col p-1 mt-4 pl-2"
              v-if="navlinks.length > 0 && allRoutes.length > 0"
            >
              <routeName
