<template>
  <div ref="dropdownRef" class="relative multiselect-dropdown" :id ="id" tabindex="0">
    <div @click="toggleDropdown" class="w-full px-4 py-1 border border-primary dark:border-gray-400 bg-secondary dark:bg-secondary-light rounded-md cursor-pointer" :class="{ 'dark:border-opacity-30': !isOpen}">
    ...
    <transition name="fade">
      <div v-if="isOpen" class="absolute z-10 mt-2 w-full max-h-60 overflow-auto text-nowrap bg-secondary border border-secondary-bold rounded-md shadow-lg">
        <div v-if="options.length" v-for="option in options" :key="option.id" :for="option.id"  class="w-full cursor-pointer transition-all hover:bg-white hover:bg-opacity-10">
          <label class="flex m-0 items-center py-[2px] pl-4 cursor-pointer">
            ...
            <div class="flex w-full m-0 items-center text-gray-700 dark:text-gray-200">
                ...
                <div class="py-[2px] capitalize">{{ option.label }}</div>
              </div>
          </label>
        </div>
        ...
        <div v-else class="w-full cursor-pointer">
          <label class="flex items-center m-0 py-[2px] pl-4 cursor-pointer">
            <span class="text-gray-700 dark:text-gray-100">Options not available</span>
          </label>
        </div>
      </div>
    </transition>
