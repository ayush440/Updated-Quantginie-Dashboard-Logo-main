<template>
    <div ref="dropdownRef" :id="id" class="relative single-select-dropdown" tabindex="0">
      <div @click="toggleDropdown" class="w-full px-4 py-1 bg-secondary-light border border-primary dark:border-gray-400  focus:outline-1 focus:outline-primary rounded"
           :class="{ 'cursor-not-allowed opacity-80': disabled, 'cursor-pointer': !disabled, 'dark:border-opacity-30': !isOpen }">
      ...
      <transition name="fade">
        ...
        <div v-if="isOpen && !disabled" class="absolute z-10 mt-[2px] w-full max-h-56 overflow-auto text-nowrap bg-secondary border border-primary dark:border-gray-400 rounded-md shadow-xl shadow-third-bold dark:shadow-primary" :style ="{ zIndex: zIndexValue }">
          <div
            ...
            :class="{ 'bg-white bg-opacity-30': option.value.toString() === selectedOption.toString() }"
          >
            <label class="flex m-0 items-center py-[2px] pl-4 cursor-pointer font-normal text-base text-tableText dark:text-white"
            >
