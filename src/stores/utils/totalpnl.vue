<template>
  ...
  <div v-if="profile && profile.user_role !== 'Paper' && grossProfit" class="font-bold" :class="grossProfit < 0 ? 'text-red-500' :grossProfit >0 ? greenColor? greenColor: 'text-green-500':''">
    {{ grossProfit>0?'+':'' }}{{ grossProfit }}
  </div>

  <div v-else-if="totalPaperProfit" class="font-bold" :class="totalPaperProfit < 0 ? 'text-red-500' :totalPaperProfit >0 ? 'text-green-500':''">
  ...
  const { isProfitIncrease } = storeToRefs(positionsStore);
  ...
  let totalPaperProfit: any =0
    ...
    watchEffect (() => {
      if (profile.value && profile.value.user_role !== 'Paper') {
        isProfitIncrease.value = grossProfit && grossProfit.value >= 0 ? true : false
      } else {
        isProfitIncrease.value = totalPaperProfit && totalPaperProfit.value >= 0 ? true : false
      }
    })
  </script>
