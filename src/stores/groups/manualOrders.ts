...
export const useManualOrdersStore = defineStore('manualOrders', () => {
  ...
  const updatedManualOrderTime = ref('')
  ...
  const getManualOrders = async () => {
    ...
    try {
      ...
      if (response.data){
        ...
        getLatestDate(manualOrders.value)
  ...
  function getLatestDate(dataArray: { updated_at: string }[]) {
    ...
    updatedManualOrderTime.value  = latestDate.updated_at
  }
