...
export const useManualPositionsStore = defineStore('manualPositions', () => {
  ...
  const showManualCloseModal = ref<boolean>(false)
  const manualDataForSqOff = ref<any>({ position_id: 0 })


  const updatedManualPositionTime = ref('')
  ...
  const getManualPositions = async () => {
    ...
    try {
      ...
      if (response.data){
        ...
        getLatestDate(manualPositions.value)
  ...
  function getLatestDate(dataArray: { updated_at: string }[]) {
    if(dataArray.length === 0) {
      updatedManualPositionTime.value  = ''
      return
    } 
    ...
    updatedManualPositionTime.value  = latestDate.updated_at
  }
